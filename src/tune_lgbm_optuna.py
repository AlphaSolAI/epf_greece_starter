"""
Optuna-based Bayesian Hyperparameter Optimization for LightGBM (Open-Loop Price/Load).
...
as lgbm_{mode}_{task}_optuna.pkl.

Usage
-----
python -m src.tune_lgbm_optuna hourly --task price \\
    --train_end "2025-11-30 23:00" \\
    --n_trials 60 --n_splits 4

python -m src.tune_lgbm_optuna hourly --task load \\
    --train_end "2025-11-30 23:00" \\
    --n_trials 60 --n_splits 4
"""
...
def _eval_fold(
    ...
    """Train LGBM with early stopping on one fold, return MAE on val."""
...
def _suggest_params(trial) -> dict:
    """Bayesian search space for LightGBM hyperparameters."""
    return dict(
...
def run_optuna(
    ...
    print(f"\n{'='*65}")
    print(f" OPTUNA LGBM TUNING | mode={mode} | task={task.upper()}")
    ...
    # Baseline trial (fixed default params)
    ...
    # Use a small validation tail for early stopping (last 168h of train)
    n = len(df_train)
    n_val = 168
    ...
    # Convert best_params (Optuna returns raw values, need to add objective etc.)
    lgbm_params = dict(
        objective="regression",
        random_state=42,
        subsample_freq=1,
    ...
    best_iter = int(getattr(final_model, "best_iteration_", max_estimators))
    ...
    model_path = MODELS_DIR / f"lgbm_{mode}_{task}_optuna.pkl"
    joblib.dump(final_model, model_path)

    params_out = {**lgbm_params, "n_estimators": best_iter}
    params_path = MODELS_DIR / f"lgbm_{mode}_{task}_optuna_params.json"
    ...
    log_path = TUNING_DIR / f"optuna_lgbm_{mode}_{task}.json"
...
def main() -> None:
    p = argparse.ArgumentParser(description="Optuna LGBM hyperparameter search")
    ...
    p.add_argument("--max_estimators", type=int, default=5000)
